    ...
    <>
          ...
          <div className="flex flex-col gap-2 justify-end text-[#cfdce7]">
            ...
            <div className="flex gap-1 items-end flex-wrap">
              {movie.genres?.map((genre) => (
                <span
                  key={genre.id}
                  className="py-1 px-2 font-bold text-[#bed0de] bg-[#35436799]"
                >
                  {genre.name}
                </span>
              ))}
              <span>
                {usCertification}, {movie.release_date?.split("-")[0]} |{" "}
      ...
      <div className="px-16 pb-20">
        <h2 className="text-2xl font-bold mt-6 mb-4">Top Billed Cast</h2>
          ...
          { topCast.map((actor) => (
            <div key={actor.id} className="w-[173px]">
              <div
                style={{
                  backgroundImage: `url(${
                    actor.profile_path
      ...
      <div className="px-16 pb-20">
        ...
        <h2 className="text-2xl font-bold mt-6 mb-4">Similar Movies</h2>
          {similarLoading && <p className="text-white">Loading similar movies...</p>}
        {!similarLoading && similarMovies.length > 0 ? (
          <div className="relative">
            <button className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-6 z-10 bg-[#233257] hover:bg-[#41527c] p-2 rounded-full cursor-pointer">
              <FaChevronLeft className="text-white w-6 h-6" />
            </button>

            <button className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-6 z-10 bg-[#233257] hover:bg-[#41527c] p-2 rounded-full cursor-pointer">
              <FaChevronRight className="text-white w-6 h-6" />
            </button>

            <div className="flex gap-4 w-full snap-x overflow-x-auto scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {
              similarMovies.map((similarMovie) => (
                <div key={similarMovie.id} className="w-54 flex-shrink-0">
                  <div
                    style={{
                      backgroundImage: `url(${
